...
public class JwtTokenUtil {
    ...
    public void init() {
        ...
        secret = (prop != null && !prop.isEmpty()) ? prop
               ...
               : "travel-platform-secret-key-2024!!"; // 开发环境默认值(256-bit)，生产环境务必通过环境变量覆盖
        if (secret.equals("travel-platform-secret-key-2024!!")) {
            log.warn("*** Using default JWT secret — set JWT_SECRET env var for production! ***");
        }
        signingKey = Keys.hmacShaKeyFor(secret.getBytes(StandardCharsets.UTF_8));
        log.info("JwtTokenUtil initialized successfully");
    }
